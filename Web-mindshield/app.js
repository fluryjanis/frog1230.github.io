...
// Configure Transformers.js for web browser
...
// 1. Initialize Local AI in Background
async function initModel() {
  try {
    statusEl.textContent = '🧠 Downloading on-device model (28MB, runs in RAM)...';
      ...
      'Xenova/distilbert-base-uncased-mnli',
      {
        progress_callback: (p) => {
          if (p.status === 'progress') {
            statusEl.textContent = `Loading local AI: ${Math.round((p.loaded / p.total) * 100)}%`;
          }
        }
      }
    );
    statusEl.textContent = '✅ Local AI Ready (100% Offline in Browser)';
  } catch (err) {
    statusEl.textContent = '❌ Failed to load local model.';
    console.error(err);
  }
}
initModel();

// 2. Client-Side Pattern Intent Engine
...
async function handleEvaluation() {
  ...
  if (triage.tier !== 'SCRUTINIZE') {
    displayResult(triage.tier, `${(performance.now() - startTime).toFixed(1)}ms (Client-Side Heuristic)`, triage.reason);
    return;
  }

  // If ambiguous, run local DistilBERT zero-shot classifier
  if (!classifier) {
    displayResult('PASS', 'AI Loading', 'Model is still initializing in RAM. Please wait a few seconds.');
    return;
  }
...
testBtn.addEventListener('click', handleEvaluation);
